...
if( ! -e "/root/.DeepOnion/DeepOnion.conf" ) #if there is no configue file, use the default one
{
    system("cp /root/configure.conf /root/.DeepOnion/DeepOnion.conf");
}
my $walletcmd= "DeepOniond";

#start the daemon
system("$walletcmd -daemon");
sleep(10);
system("status_report.pl >> /root/.DeepOnion/status.log &");
